...
export class Persisted<T> {
	...
	constructor(key: string, initialValue: T, options: PersistedOptions<T> = {}) {
		const {
		...
		$effect(() => {
			...
			return useEventListener(window, "storage", this.#handleStorageEvent.bind(this));
		});
	}
